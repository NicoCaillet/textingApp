...
function ContactBox(props) {
  ...
  const { id, img, name, lastMessageHour } = object;
  ...
  return (
    <Grid className={classes.contactDetail} onClick={() => getPerson(object)}>
      <Grid className={classes.divContactImg}>
        <img src={img} alt={name} className={classes.imgContact} />
      </Grid>
      <Grid className={classes.divContactName}>
        <Typography className={classes.nameContact}> {name}</Typography>
        <Typography className={classes.lastmessage}>
          {JSON.parse(localStorage.getItem(`${contacts[id].name}`)) &&
            JSON.parse(localStorage.getItem(`${contacts[id].name}`)).pop()
              .message}
        </Typography>
      </Grid>
